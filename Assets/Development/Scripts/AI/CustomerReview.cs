...
public class CustomerReview : MonoBehaviour
{
    ...
    public void GenerateReview(CustomerBase customer, float starAmount)
    {
        orderOwner = customer;
        drinkScore = starAmount;
        cafeCleanliness = starAmount;
        cafeCrowdedness = starAmount;
        timeToServe = starAmount;
        reviewScore = CalculateReviewScore(cafeCleanliness, cafeCrowdedness, timeToServe, drinkScore);
    }
    ...
    private int CalculateReviewScore(float cafeCleanliness, float cafeCrowdedness, float timeToServe, float drinkScore)
    {
        ...
        //check to see if all the aspects of the score are the same and == 1 for a leaving customer
        if (cafeCleanliness == 1 && cafeCrowdedness == 1 && timeToServe == 1 && drinkScore == 1)
        {
            System.Random random = new System.Random();
            int randIndex = random.Next(3);
            if (randIndex == 1)
                reviewText = "This place is the slowest coffee shop I have EVER been to";
            else if (randIndex == 2)
                reviewText = "I should have stopped at Dark Matter Doughnuts instead :(";
            else
                reviewText = "I waited for SO LONG, had to leave to check if my oven was still on";
            Debug.Log("1 star Review Activated");
        }
        else
        {
